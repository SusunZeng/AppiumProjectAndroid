...
作者：曾志坤，时间：20180408
参考：https://www.cnblogs.com/syw20170419/p/6964339.html
'''
...
def classify_aHash(image1, image2):
    ...
    gray1 = cv2.cvtColor(image1,
                         cv2.COLOR_BGR2GRAY)  # cv2.cvtColor(input_image,flag)实现图片颜色空间的转换，flag 参数决定变换类型。如 BGR->Gray flag 就可以设置为 cv2.COLOR_BGR2GRAY 。
...
if __name__ == '__main__':
    img1 = cv2.imread('E:\\p1\\1.png',
                      cv2.IMREAD_COLOR)  # 读入图片，共2个参数，第一个参数为要读入的图片文件名，第二个参数为如何读取图片，包括cv2.IMREAD_COLOR读入一幅彩色图片,cv2.IMREAD_UNCHANGED读入一幅彩色图片，并包括alpha通道，cv2.IMREAD_GRAYSCALE已灰度模式读入图片
